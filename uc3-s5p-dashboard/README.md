...
output$timeseries <- renderPlot({
      ...
      cloud = input$cloud
```
After that, the algorithms are written inside the rshiny app. Those have been already explained before. For the "Map Maker", the main difference is that a bounding box is not defined, but the input is *textInput()* of the country of interest. You can check the differences below :
...
The UDF defined for this rshiny app is a Moving Average one with 30 days window size. The UDF is defined as [a python script](src/udf.py) with a simple implementation of a moving average. In order to call the UDF python script, one we basically needs to make use of the process called *run_udf()*, as below :
